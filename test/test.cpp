...
void test_get_set() {
  DB* db = nullptr;
  DB::CreateOrOpen("./tmp", nullptr, &db);
  Slice k;
  k.size() = 16;
  Slice v;
...
void test2() {
  DB* db = nullptr;
  DB::CreateOrOpen("./tmp", nullptr, &db, log_file);
  Slice k;
  k.size() = 16;
  Slice v;
  v.size() = 80;
...
void test3() {
  DB* db = nullptr;
  DB::CreateOrOpen("/mnt/pmem/DB", nullptr, &db, log_file);
  Slice k;
  k.size() = 16;
  Slice v;
  ...
  for (int i = 0; i < compare.size(); i += 2) {
    ...
    if (0 == strcmp(a.data(), compare.at(i + 1).data())) {
      ...
      printf("set key %s \n", compare.at(i).data());
      printf("Set val %s size %d\n", compare.at(i + 1).data(), compare.at(i +
  1).size()); printf("Get val %s size %d\n", a.data(), a.size());
    }
  }
